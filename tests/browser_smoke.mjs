...
try {
  ...
  assert.equal(await evaluate('$("glycans_O").value'), '');
  assert.equal(await evaluate('$("glycans_N").value'), '');
  assert.equal(await evaluate('document.querySelectorAll("#glycan-library .protein-label").length'), 0);
  assert.match(await evaluate('document.querySelector("input[value=o_tn]").closest(".glycan-card").textContent'), /221\.0899546/);
  await evaluate('$("select-all-glycans").click()');
  assert.equal(await evaluate('document.querySelectorAll(".glycan-card input:checked").length'), 21);
  assert.equal(await evaluate('$("library-count").textContent'), '21 SELECTED');
  assert.equal(await evaluate('$("select-all-glycans").disabled'), true);
  await evaluate('document.querySelector("input[value=o_tn]").click()');
  assert.equal(await evaluate('selectedGlycans.size'), 20);
  assert.equal(await evaluate('$("select-all-glycans").disabled'), false);
  await evaluate('$("select-all-glycans").click();$("clear-glycans").click()');
  assert.equal(await evaluate('document.querySelectorAll(".glycan-card input:checked").length'), 0);
  assert.equal(await evaluate('selectedGlycans.size'), 0);
  assert.equal(await evaluate('$("clear-glycans").disabled'), true);
  ...
  assert.equal(await evaluate('document.querySelector("#structure-content .protein-label").textContent'), 'Ser/Thr');
  ...
  assert.equal(await evaluate('document.querySelector("#structure-content .protein-label").textContent'), 'Asn');
  await screenshot('/tmp/glycotag-complex-structure.png');
  await evaluate('$("close-structure").click();document.querySelector("input[value=o_core1]").click();$("glycan_mass_type").value="attached";$("glycan_mass_type").dispatchEvent(new Event("change",{bubbles:true}))');
  assert.equal(await evaluate('document.querySelector("input[value=o_core1]").checked'),true);
  assert.match(await evaluate('document.querySelector("input[value=o_core1]").closest(".glycan-card").textContent'),/365\.1321899/);
  ...
  await evaluate(`
    ...
    const m=catalog.glycans.find(g=>g.id==='o_core1').attached_mass+87.03202840472+18.010564684;
  ...
  assert.match(await evaluate('$("calculation-body").textContent'),/18\.010564684/);
  assert.match(await evaluate('$("calculation-body").textContent'),/terminal H₂O/);
  assert.match(await evaluate('$("calculation-body").textContent'),/Signed Δm\/z/);
  ...
  await evaluate('$("select-all-glycans").click()');
  assert.equal(await evaluate('last'), null);
  assert.equal(await evaluate('$("results").hidden && $("calculation").hidden'), true);
  await evaluate('$("clear-glycans").click();document.querySelector("input[value=o_core1]").click();$("form").requestSubmit()');
  await waitFor('!$("results").hidden && !$("run").disabled');
  ...
  assert.match(csv,/aa_water_loss/);assert.match(csv,/o_core1/);assert.match(csv,/attached/);
  console.log('PASS: CSV contains calculation totals and glycan selection.');
  assert.match(csv,/absolute_delta_mz/);assert.match(csv,/signed_delta_mz/);
  assert.match(csv,/target_neutral_mass/);assert.match(csv,/ion_mode/);assert.match(csv,/positive/);
  await evaluate(`$('ion_mode').value='negative';$('ion_mode').dispatchEvent(new Event('change',{bubbles:true}));`);
  assert.equal(await evaluate('$("results").hidden && $("calculation").hidden'),true);
  await evaluate(`const neutral=catalog.glycans.find(g=>g.id==='o_core1').free_mass+87.03202840472;fill('observations',String(neutral/4-1.007276466621)+',4');$('form').requestSubmit()`);
  await waitFor('!$("results").hidden && !$("run").disabled');
  assert.ok(Math.abs(await evaluate('last.results[0].rows[0].signed_delta_mz'))<1e-10);
  assert.match(await evaluate('$("result-list").textContent'),/charge 4−/);
  assert.match(await evaluate('$("calculation-body").textContent'),/Negative/);
  assert.match(await evaluate('$("calculation-body").textContent'),/Target neutral mass/);
  const attachedMz=await evaluate('last.results[0].rows[0].predicted_mz');
  await evaluate(`$('glycan_mass_type').value='free';$('glycan_mass_type').dispatchEvent(new Event('change',{bubbles:true}));$('form').requestSubmit()`);
  await waitFor('!$("results").hidden && !$("run").disabled');
  assert.ok(Math.abs(await evaluate('last.results[0].rows[0].predicted_mz')-attachedMz)<1e-10);
  await evaluate('$("download").click()');
  assert.match(await evaluate('savedBlob.text()'),/negative/);
  console.log('PASS: positive/negative ion modes, neutral targets, free/attached equivalence, Δm/z and CSV settings.');
